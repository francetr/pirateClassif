...
However, it requires the ***biopython*** modules (usefull to write the sequences onto fasta files), which you can acces with ***pip***. It also requires the package ***fastx-toolkit***, that can be install with the command :  
...
## Architecture :
This script can be integrated onto the VM containing PiRATE, thus into the VM, there is a certain architecture to respect if you want that this script work properly.  
![Architecture](./diagrammes/Diagramme_Architecture.jpeg)
...
We differenciate keyword designated superFamily names and those designated proteine profiles, which begin by the _>_ character, like in the example below.  
> Example :  
...
~~~{bash}
python PiRATEclassif.py ArabiTEdenovo.txt ArabiTEdenovo.fasta
python3 PiRATEclassif.py ArabiTEdenovo.txt ArabiTEdenovo.fasta
python3 PiRATEclassif.py ArabiTEdenovo.txt ArabiTEdenovo.fasta -e 75
python3 PiRATEclassif.py ArabiTEdenovo.txt ArabiTEdenovo.fasta --baseline new_base_reference.txt
python3 PiRATEclassif.py ArabiTEdenovo.txt ArabiTEdenovo.fasta -e 75 --baseline new_base_reference.txt
~~~
...
> ./path/toward/the/PiRATEclassif.py&nbsp;&nbsp;path/toward/the/classif/file path/toward/the/fasta/file  
> example:

~~~{bash}
./PiRATEclassif.py ArabiTEdenovo.txt ArabiTEdenovo.fasta
~~~
...
It will then creates automatically different prelibraries containing fasta sequences according the classification information attributed to the sequence. Hence, 4 types of prelibraries are created:  
* **saveType** : gather sequences classified with the same saveType (**noCat** or **nonTE** or **potentialChimeric**)  

For the TE sequences, a repository is created containing prelibraries ordered according their finalDegree classification:
* **class** : folder gathering TE sequences classified with the same class (**I** or **II**)
* **order** : folder gathering TE sequences classified with the same order (**TIR**, **LTR**, etc)
* **superFamily** : folder gathering TE sequences classified with the same superFamily (**Copia**, **Gypsy**, etc)

### Creation of prelibraries :
For the saveType prelibraries, four outcomes are possible:
1. **noCat** : file with sequences that cannot be categorized
2. **nonTE** : file with sequences that are not Transposable Element (can be PotentialHostGene)
3. **PotentialChimeric** : file with sequences that cannot be categorized as 2 or more superFamilies
4. **TE** : the superFamily's sequence has been established. According the finalDegree given for each sequences, a prelibrarie will be created containing all the sequences which have been classified with this finalDegree.



### Other outcomes to understand the classification :
...
> ltrharvest_941 &emsp; 10243 &emsp; incomplete &emsp; TE &emsp; CLASS:&emsp;I &emsp;Gypsy&emsp;ORDER:&emsp;LTR &emsp; BLAST : {'Gypsy': &nbsp; [6, 100.0]} &emsp; PROTPROFILES : &nbsp; {'RH': &nbsp; [1, 20.0], 'AP': &nbsp; [1, 20.0], 'RT': &nbsp; [2, 40.0], 'GAG': &nbsp; [1, 20.0]} &emsp; PREDICTED_SUPERFAMILY: &emsp; Gypsy  &emsp; FINALDEGREE:
> From right to left we got :  
name of the sequence &emsp; length &emsp; completeness &emsp; type of the sequence(TE, nonTE, potentialChimeric or noCat) &emsp; class &emsp; order &emsp; Blast keywords: &nbsp; name: [nb of occurrence, &nbsp; percentage], &nbsp; Proteine Profile keywords: name:[nb of occurrence, percentage] &emsp; name predicted of the superFamily &emsp; Highest degree according Wicker's classifiaction
...
Once the superFamily for the TEs are created and the prelibraries builded, it is needed to determined the last level of classification according Wicker, which the the family. For that purpose, we use the ***MCL*** algorithm that is implemented in the ***REPET*** package, specifically dedicated to the TEs.  
So far, this script will only work in the VM containing ***PiRATE*** and cannot be executed outside. To work, it also needed to respect an architecture (see part _Architecture_ above).  
